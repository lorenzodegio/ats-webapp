...
def fase_ocr():
    ...
    # FIX: pipeline.py (passo 2/4) prevede esegui_merge_regione() prima
    # della difformita — i check 14/17/18 dipendono dai campi LORDO_PRESC
    # e CONTROLLO_CODICE_PRESCRIZIONE che aggiunge. Nella versione
    # originale di questo dispatcher non veniva mai chiamata: la fase
    # difformita partiva quindi sempre senza dati Regione disponibili.
    regione_eseguita = pipeline.esegui_merge_regione(excel_regione, output_dir)
    if not regione_eseguita:
        print("ATTENZIONE: arricchimento Regione saltato (nessun file Excel Regione trovato in /dati/excel_regione)")
