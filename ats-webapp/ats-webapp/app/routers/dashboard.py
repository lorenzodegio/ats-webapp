...
STATI_IN_CORSO = [
    s for s in StatoLotto
    if s not in (StatoLotto.completato, StatoLotto.archiviato, StatoLotto.eccezione)
]
...
def dashboard(
    ...
    if stato == "in_corso":
        query_lotti = query_lotti.filter(LottoMensile.stato.in_(STATI_IN_CORSO))
    elif stato and stato in StatoLotto.__members__:
