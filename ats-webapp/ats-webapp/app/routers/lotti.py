...
from app.models import (
    ...
    StatoElaborazione, FaseElaborazione, DatiOcr, Utente,
)
from app.fake_pipeline import (
    avvia_preprocessing_fake, avvia_ocr_fake, avvia_difformita_fake, genera_excel_output_fake,
    FAKE_SP_ROOT, genera_pdf_finto, costruisci_riga_output, COLONNE_ANTEPRIMA_OUTPUT,
    COLONNE_OUTPUT_EXCEL, PERCORSO_CARTELLA_OUTPUT_RECENTI,
)
...
def dettaglio_lotto(
    ...
    anteprima_output = []
    if lotto.stato in (StatoLotto.completato, StatoLotto.archiviato) and lotto.excel_output_filename:
        anteprima_output = [costruisci_riga_output(p, lotto) for p in lotto.prescrizioni[:20]]
    ...
    return templates.TemplateResponse(
        "lotto_detail.html",
        {
            ...
            "anteprima_output": anteprima_output,
            "colonne_anteprima_output": COLONNE_ANTEPRIMA_OUTPUT,
            "numero_colonne_output": len(COLONNE_OUTPUT_EXCEL),
        },
    )
...
def visualizza_pdf_prescrizione(
    ...
    entro_la_radice = percorso.startswith(radice + os.sep)

    if entro_la_radice and os.path.isfile(percorso):
        nome_file = os.path.basename(percorso)
        return FileResponse(
            percorso, media_type="application/pdf",
            headers={"Content-Disposition": f'inline; filename="{nome_file}"'},
        )

    # Fallback: il record esiste nel DB ma il file fisico no — tipicamente
    # perche' la prescrizione arriva da seed_admin.py, che popola solo il
    # database senza scrivere nulla su sharepoint_finto/ (vedi
    # HANDOFF_FRONTEND.md). Invece di un errore, generiamo al volo un PDF
    # segnaposto con gli stessi dati, cosi' il link funziona sempre in
    # sviluppo. Provvisorio: da togliere quando ogni prescrizione avra'
    # davvero un file dietro (backend/storage reale).
    pdf_bytes = genera_pdf_finto([
        "PRESCRIZIONE - DOCUMENTO SIMULATO (ambiente di sviluppo)",
        f"Lotto: {presc.lotto.nome if presc.lotto else '—'}",
        f"Barcode: {presc.barcode or 'non ancora letto'}",
        f"Stato barcode: {presc.stato_barcode.value.replace('_', ' ')}",
        "",
        "File fisico non trovato su sharepoint_finto/: probabilmente questa",
        "prescrizione arriva da seed_admin.py, che popola solo il database.",
        "Questa e' un'anteprima generata al volo per non bloccare il link.",
    ])
    return Response(
...
def visualizza_output_excel(
    ...
    if not percorso.startswith(radice + os.sep) or not os.path.isfile(percorso):
        return JSONResponse({"errore": "File non trovato su sharepoint_finto/"}, status_code=404)

    return FileResponse(
        percorso,
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={"Content-Disposition": f'inline; filename="{lotto.excel_output_filename}"'},
    )
...
def riprova_lotto(
    ...
    if fase == FaseElaborazione.preprocessing:
        ...
        lotto.n_prescrizioni_con_difformita = 0
        db.commit()
        background_tasks.add_task(avvia_difformita_fake, lotto.id)

    else:
        # fase sconosciuta/non gestita: torniamo comunque a uno stato
        # sicuro invece di lasciare il lotto bloccato in eccezione.
        lotto.stato = StatoLotto.caricamento
        db.commit()
...
def completa_lotto(
    ...
    if lotto and lotto.stato == StatoLotto.revisione_difformita:
        # Genera il foglio di output finale PRIMA di segnare il lotto
        # completato, cosi' se la scrittura fallisse il lotto resterebbe
        # nello stato precedente invece di risultare "completato" senza
        # un file dietro.
        genera_excel_output_fake(lotto.id)
        db.refresh(lotto)
