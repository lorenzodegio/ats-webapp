...
from app.fake_pipeline import (
    scrivi_pdf_finto, costruisci_workbook_output, nome_file_output_excel,
    salva_workbook_output, PERCORSO_CARTELLA_OUTPUT_RECENTI,
)
...
def _crea_prescrizioni(db, lotto, n, con_undefined=False, con_ocr=False, con_difformita=False, creato_il=None):
    ...
    for i in range(n):
        ...
        scrivi_pdf_finto(presc.sp_pdf_path, [
            "PRESCRIZIONE - DOCUMENTO SIMULATO (dati demo, seed_admin.py)",
            f"Lotto: {lotto.nome}",
...
def popola_lotti_demo(db, operatore):
    ...
    for mese, anno, stato, giorni_fa in scenari:
        ...
        if stato in (StatoLotto.completato, StatoLotto.archiviato):
            # Come nel flusso reale (completa_lotto), l'excel di output
            # viene scritto per davvero, non solo referenziato in DB —
            # altrimenti il bottone "Apri file Excel" darebbe 404 sui
            # lotti demo, come gia' successo con i PDF delle prescrizioni.
            wb = costruisci_workbook_output(lotto, lotto.prescrizioni)
            nome_file = nome_file_output_excel(lotto)
            salva_workbook_output(f"{PERCORSO_CARTELLA_OUTPUT_RECENTI}/{nome_file}", wb)
